require 'uri'
require 'net/http'
class PtTaskUploadJob < ApplicationJob
  queue_as :default

  after_perform do |job|
    hash = job.arguments.first
    PtTaskCheckUploadProgressJob.perform_later(hash)
  ...
  def perform(*args)
    ...
    res = cfetch(ENV['RSYNC_UPLOAD_HOST'] + '/upload?hash=' + encodeUri(hash) + '&fpath=' + encodeUri(fpath))
  end
